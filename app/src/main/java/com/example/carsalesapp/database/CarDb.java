    ...
    public static CarDb getDatabase(final Context context)
    {
        if (INSTANCE == null)
        {
            synchronized (CarInfoDao.class) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder( context.getApplicationContext(), CarDb.class, "cars1")
    ...
    private static RoomDatabase.Callback sRoomDatabaseCallback =  new RoomDatabase.Callback() {
        ...
        public void onCreate(@NonNull SupportSQLiteDatabase db) {
            ...
            databaseWriteExecutor.execute(() -> {
                ...
                CarEntity car = new CarEntity("326", "BMW", "Leaking oil", 2100.0,"admin@admin");
                dao.insert(car);
                car = new CarEntity("leon", "seat", "Check engine light on", 300.0,"user@user");
                dao.insert(car);
            });
        }
    };
